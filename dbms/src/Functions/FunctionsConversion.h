                ...
                (Block & block, const ColumnNumbers & arguments, const size_t result, size_t input_rows_count)
        {
            ...
            if (to_low_cardinality)
            {
                ...
                if (from_low_cardinality && !src_converted_to_full_column)
                {
                    ...
                    col_low_cardinality_->insertRangeFromDictionaryEncodedColumn(*res_keys, *res_indexes);
                }
                else
                    col_low_cardinality_->insertRangeFromFullColumn(*res.column, 0, res.column->size());
